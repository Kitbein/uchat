...
void theme_marvel_click(GtkWidget *widget, GdkEventButton *event, gpointer menu) {
...
void theme_anime_click(GtkWidget *widget, GdkEventButton *event, gpointer menu) {
...
void theme_game_click(GtkWidget *widget, GdkEventButton *event, gpointer menu) {
...
void account_inf_click(GtkWidget *widget, GdkEventButton *event) {
    if(widget && t_settings_screen.active == 1) return;
    if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
        gtk_widget_unset_state_flags(GTK_WIDGET(t_settings_screen.boxes[t_settings_screen.active-1]), GTK_STATE_FLAG_LINK);
        t_settings_screen.active = 1;
        gtk_widget_set_state_flags(GTK_WIDGET(t_settings_screen.boxes[t_settings_screen.active-1]), GTK_STATE_FLAG_LINK, TRUE);

        gtk_widget_hide(GTK_WIDGET(t_settings_screen.active_screen));
        t_settings_screen.active_screen = t_settings_screen.account_block;
        gtk_widget_show(GTK_WIDGET(t_settings_screen.active_screen));
    }
}

void theme_settings_click(GtkWidget *widget, GdkEventButton *event) {
    if(widget && t_settings_screen.active == 2) return;
    if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
        gtk_widget_unset_state_flags(GTK_WIDGET(t_settings_screen.boxes[t_settings_screen.active-1]), GTK_STATE_FLAG_LINK);
        t_settings_screen.active = 2;
        gtk_widget_set_state_flags(GTK_WIDGET(t_settings_screen.boxes[t_settings_screen.active-1]), GTK_STATE_FLAG_LINK, TRUE);

        gtk_widget_hide(GTK_WIDGET(t_settings_screen.active_screen));
        t_settings_screen.active_screen = t_settings_screen.theme_block;
        gtk_widget_show(GTK_WIDGET(t_settings_screen.active_screen));
    }
}

void exit_fr_settings_click(GtkWidget *widget, GdkEventButton *event) {
    if(widget){}
    if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
        gtk_widget_unset_state_flags(GTK_WIDGET(t_settings_screen.boxes[t_settings_screen.active-1]), GTK_STATE_FLAG_LINK);
        t_settings_screen.active = 1;
