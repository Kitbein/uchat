...
void login_butt_click(GtkWidget *widget){
    ...
    if (!cJSON_IsNull(response) && strcmp(result, "")) {
        ...
        t_screen.active = 2;   // Active button in leftbar
        ...
        gtk_widget_hide(GTK_WIDGET(t_msg.scrolled_message));
    gtk_widget_hide(GTK_WIDGET(t_msg.entry_block));
    }
    else {
