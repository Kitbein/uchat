...
void adduser_click(GtkWidget *widget, GdkEventButton *event, gpointer search_field) {
    ...
    if(event->type == GDK_BUTTON_PRESS && event->button == 1)
    {
        ...
        if (!add_user_to_group(name, &user_id, &avatar)) {
            create_notification(t_application.messanger, "Invalid username!", 1, 10, 58, 340, 10);
            return;
        }
        ...
        if (!chat_id) {
            create_notification(t_application.messanger, "You already have DM with this user!", 1, 10, 58, 340, 10);
            return;
        }
...
void person_click(GtkWidget *widget, GdkEventButton *event) {
    ...
    {
        gtk_widget_show(GTK_WIDGET(t_msg.scrolled_message));
        gtk_widget_show(GTK_WIDGET(t_msg.entry_block));
        gtk_widget_hide(GTK_WIDGET(t_screen.theme_screen));
...
void settings_click(GtkWidget *widget, GdkEventButton *event) {
    ...
    if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
        ...
        gtk_widget_hide(GTK_WIDGET(t_settings_screen.theme_block));
    }
}
